...
class GeneralSettings(BaseModel):
    ...
    master_key: SecretStr = Field(description="Master key for LiteLLM admin API (generated on first setup)")
...
class GatewayConfig(BaseModel):
    ...
    def validate_master_key(cls, v: str) -> str:
        if isinstance(v, str) and len(v) < 32:
            raise ValueError("master_key must be at least 32 characters for security")
        return v
