...
def choose_ui_password() -> None:
    """Wizard question 6: UI admin password (hashed into Postgres later)."""
    import getpass
    print("\n=== Step 6/7: Web UI Admin Password ===")
    while True:
        pw = getpass.getpass("  Choose UI admin password (min 8 chars, blank to set on first UI visit): ").strip()
        if not pw:
...
def generate_env_file(env_path: Path, keys: dict, mode: str = "docker") -> None:
    ...
    lines = [
        ...
        "",
        *[f"{k}={v}" for k, v in sorted(keys.items()) if k != "POSTGRES_PASSWORD"],
        "",
        ...
        "# Random per-boot UI session secret — set a fixed value for persistent sessions",
        f"SESSION_SECRET={secrets.token_urlsafe(32)}",
        "",
        "# LiteLLM master key (randomly generated per plan Issue 12)",
        f"LITELLM_MASTER_KEY=sk-litellm-{secrets.token_urlsafe(32)}",
        "",
    ]
...
def main() -> int:
    ...
    # Idempotency: never overwrite without explicit confirmation
    if env_path.exists() and config_path.exists() and not args.force:
    ...
    # Q1: API keys
    if args.scripted and args.api_keys:
    ...
    # Q6: UI admin password (needs DATABASE_URL from .env just written)
    ...
    if deploy_mode == "docker":
        ...
        print("  2. Run: docker compose --profile full up -d")
